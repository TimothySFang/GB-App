...
import { Hold, HoldRole, Climb, WallVersion } from '@/lib/types';
...
export function Dashboard() {
  ...
  const selectedVersion = data.wall.versions.find((v) => v.id === selectedVersionId) ?? latest;
  ...
  const highlightedDraftHolds = selectedVersion.holds
    ...
    .map((hold) => {
      const role = (['start', 'middle', 'finish'] as HoldRole[]).find((candidate) =>
        draft.selected[candidate].includes(hold.canonicalHoldId)
      );
      return { hold, role };
    });
  ...
  const roleSummary = (role: HoldRole) =>
    draft.selected[role]
      .map((holdId) => selectedVersion.holds.find((hold) => hold.canonicalHoldId === holdId)?.label)
      .filter(Boolean)
      .join(', ') || 'None selected';
    ...
    <div className="container col mobile-shell">
      <div className="card">
        ...
        <div className="row">
          ...
          <span className="badge">{data.users.length} members</span>
        </div>
      </div>
      ...
      {activeTab === 'home' && (
        <div className="card col">
          <div className="row" style={{ justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <VersionSelect versions={data.wall.versions} value={selectedVersionId} onChange={setSelectedVersionId} />
          </div>
          ...
          <div className="col">
            ...
            <div className="list">
              {visibleClimbs.map((climb) => {
                const baseVersion = data.wall.versions.find((v) => v.id === climb.wallVersionId) ?? latest;
          ...
          <div className="card col">
            ...
            <div className="grid grid-2 mobile-grid-1">
              ...
              <div>
                ...
                <div className="row">
                  {(['start', 'middle', 'finish'] as HoldRole[]).map((role) => (
                    <button key={role} className={`button ${selectedRole === role ? '' : 'secondary'}`} type="button" onClick={() => setSelectedRole(role)}>{role}</button>
                  ))}
                </div>
              </div>
            </div>
      ...
      {activeTab === 'versions' && (
        <div className="col">
          <div className="card"><h2 className="section-title">Wall versions</h2><p className="section-subtitle">Track resets and whether older climbs still work.</p></div>
          <div className="list">
            {data.wall.versions.map((version) => (
              <div key={version.id} className="card col">
      ...
      <nav className="bottom-nav">
        {[
          ...
          <button key={id} className={`bottom-nav-item ${activeTab === id ? 'active' : ''}`} onClick={() => setActiveTab(id as TabId)}>{label}</button>
        ))}
      </nav>
    </div>
  );
...
function VersionSelect({ versions, value, onChange }: { versions: WallVersion[]; value: string; onChange: (value: string) => void }) {
  return (
    <select className="select compact-select" value={value} onChange={(e) => onChange(e.target.value)}>
...
function ClimbDetail({
  ...
  latestVersion: WallVersion;
  versions: WallVersion[];
  compatibility: typeof mockData.compatibility;
  ...
  const highlight = baseVersion.holds
    .filter((hold) => climb.holds.some((ref) => ref.holdId === hold.canonicalHoldId))
    .map((hold) => ({ hold, role: climb.holds.find((ref) => ref.holdId === hold.canonicalHoldId)?.role }));
  ...
  return (
    <div className="wall-preview interactive-wall">
      ...
      {version.holds.map((hold: Hold) => {
        ...
        return (
          <button key={hold.id} type="button" className={`hold-button ${isSelected ? 'selected' : ''}`} title={`${hold.label} · add as ${selectedRole}`} style={{ left: `${hold.x}%`, top: `${hold.y}%`, background: hold.color }} onClick={() => onToggleHold(hold.canonicalHoldId)}>
